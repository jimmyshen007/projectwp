...
function epl_get_author_sticker(){
	global $epl_author;
	$aurl = get_avatar_url($epl_author->author_id);
	return '<span><img style="border: solid white 2px;" class="img-circle" \
		src="' . $aurl . '" width="50px" height="50px"' . '/></span>';
}
		...
		<div class="epl-archive-entry-image">
			<a href="<?php the_permalink(); ?>">
				<div class="epl-blog-image">
					...
					<!-- Since author sticker div has to be moved along with image -->
					<div class="epl-author-sticker">
						<?php echo epl_get_author_sticker(); ?>
					</div>
					<?php the_post_thumbnail( $image_size , array( 'class' => $image_class ) ); ?>
				</div>
			</a>
		...
		<div class="epl-archive-entry-image">
			<a href="<?php the_permalink(); ?>">
				<div class="epl-blog-image">
					...
					<img width="300" height="200" src="/wordpress/wp-content/plugins/custom_support/images/no_photo_ph.jpg"
	...
	<div id="Gallery-Carousel" class="carousel slide" data-ride="carousel" data-interval="false">
	  ...
	  <ol class="carousel-indicators"> <?php
		for($i=0; $i < count($attachments); $i++){
			...
			echo '<li data-target="#Gallery-Carousel" data-slide-to="' . $i . '"' . $active_class . '></li>';
	  ...
	  <div class="carousel-inner" role="listbox">
		  ...
		  foreach ( $attachments as $id => $attachment ) {
			  ...
			  if (!empty($atts['link']) && 'file' === $atts['link']) {
				  ...
				  $image_src = wp_get_attachment_image_src($id, array(400, 300), false)[0];
			  } else {
				  $image_output = wp_get_attachment_link($id, $atts['size'], true, false, false, $attr);
			  }

              if($image_src){
              	$image_output = '<img class="img-responsive center-block" src="' . $image_src .
              	 	'" height="' . $img_height . 'px" style="height: ' . $img_height . 'px">';
              }
			  ...
			  echo '<div class="' . $item_class . '">' . $image_output . '</div>';
...
function epl_property_gallery () {
	...
	if ( $attachments && $d_gallery == 1 ) { ?>
		<div class="epl-gallery property-gallery">
			<!-- Gallery -->
			<?php
				$d_gallery_n = '[custom_gallery columns="'. $d_gallery_n . '"  link="none"]';
...
/**
 * @hooked the_property_contact
 **/
function epl_property_contact(){
	global $property;
	$contacts = $property->get_property_contact();
	$contents = '<br/><ul style="list-style-type:none">';
	if($contacts["pphone"]){
		$contents .= '<li><i class="fa fa-phone-square"></i> ' . $contacts["pphone"] . '<br/></li>';
	}
	if($contacts["pwechat"]){
		$contents .= '<li><i class="fa fa-weixin"></i> ' . $contacts["pwechat"] . '<br/></li>';
	}
	if($contacts["pemail"]){
		$contents .= '<li><i class="fa fa-envelope"></i><a href="mailto:'
                  . $contacts["pemail"] . '" target="_top"> ' . $contacts["pemail"] . '</a><br/></li>';
	}
	// No contact, then tell user.
	if(!strpos($contents, '<li>')){
		$contents .= '<li>' . __('This guy is lazy, no contact left, but you can reply to this post.', 'epl') . '</li>';
	}
	$contents .= "</ul><br/><br/>";
	echo $contents;
}
add_action('epl_property_contact','epl_property_contact');
