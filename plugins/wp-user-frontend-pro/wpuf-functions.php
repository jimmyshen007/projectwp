 ...
 */
function wpuf_states_fields( $form_id, $post_id, $form_settings ) {
    $value = '';
    function setselect($cvalue, $avalue){
         $valuestr = $cvalue == $avalue ? ' selected="selected"' : '';
         return $valuestr;
    }
    if ( $post_id ) {
        $value = get_post_meta( $post_id, 'property_address_state', true );
    }
    ...
    ?>
        ...
        <div id="state_field_USA" class="wpuf-fields">
            <select class="form-control" id="state_field_select_USA" name="wpuf_address_state_field_USA" data-required="yes" data-type="select">
                ...
                <option value="AZ"<?=setselect('AZ', $value)?>>Arizona</option>
                <option value="AR"<?=setselect('AR', $value)?>>Arkansas</option>
                <option value="CA"<?=setselect('CA', $value)?>>California</option>
                <option value="CO"<?=setselect('CO', $value)?>>Colorado</option>
                <option value="CT"<?=setselect('CT', $value)?>>Connecticut</option>
                <option value="DE"<?=setselect('DE', $value)?>>Delaware</option>
                <option value="FL"<?=setselect('FL', $value)?>>Florida</option>
                <option value="GA"<?=setselect('GA', $value)?>>Georgia</option>
                <option value="HI"<?=setselect('HI', $value)?>>Hawaii</option>
                <option value="ID"<?=setselect('ID', $value)?>>Idaho</option>
                <option value="IL"<?=setselect('IL', $value)?>>Illinois</option>
                <option value="IN"<?=setselect('IN', $value)?>>Indiana</option>
                <option value="IA"<?=setselect('IA', $value)?>>Iowa</option>
                <option value="KS"<?=setselect('KS', $value)?>>Kansas</option>
                <option value="KY"<?=setselect('KY', $value)?>>Kentucky</option>
                <option value="LA"<?=setselect('LA', $value)?>>Louisiana</option>
                <option value="ME"<?=setselect('ME', $value)?>>Maine</option>
                <option value="MD"<?=setselect('MD', $value)?>>Maryland</option>
                <option value="MA"<?=setselect('MA', $value)?>>Massachusetts</option>
                <option value="MI"<?=setselect('MI', $value)?>>Michigan</option>
                <option value="MN"<?=setselect('MN', $value)?>>Minnesota</option>
                <option value="MS"<?=setselect('MS', $value)?>>Mississippi</option>
                <option value="MO"<?=setselect('MO', $value)?>>Missouri</option>
                <option value="MT"<?=setselect('MT', $value)?>>Montana</option>
                <option value="NE"<?=setselect('NE', $value)?>>Nebraska</option>
                <option value="NV"<?=setselect('NV', $value)?>>Nevada</option>
                <option value="NH"<?=setselect('NH', $value)?>>New Hampshire</option>
                <option value="NJ"<?=setselect('NJ', $value)?>>New Jersey</option>
                <option value="NM"<?=setselect('NM', $value)?>>New Mexico</option>
                <option value="NY"<?=setselect('NY', $value)?>>New York</option>
                <option value="NC"<?=setselect('NC', $value)?>>North Carolina</option>
                <option value="ND"<?=setselect('ND', $value)?>>North Dakota</option>
                <option value="OH"<?=setselect('OH', $value)?>>Ohio</option>
                <option value="OK"<?=setselect('OK', $value)?>>Oklahoma</option>
                <option value="OR"<?=setselect('OR', $value)?>>Oregon</option>
                <option value="PA"<?=setselect('PA', $value)?>>Pennsylvania</option>
                <option value="RI"<?=setselect('RI', $value)?>>Rhode Island</option>
                <option value="SC"<?=setselect('SC', $value)?>>South Carolina</option>
                <option value="SD"<?=setselect('SD', $value)?>>South Dakota</option>
                <option value="TN"<?=setselect('TN', $value)?>>Tennessee</option>
                <option value="TX"<?=setselect('TX', $value)?>>Texas</option>
                <option value="UT"<?=setselect('UT', $value)?>>Utah</option>
                <option value="VT"<?=setselect('VT', $value)?>>Vermont</option>
                <option value="VA"<?=setselect('VA', $value)?>>Virginia</option>
                <option value="WA"<?=setselect('WA', $value)?>>Washington</option>
                <option value="WV"<?=setselect('WV', $value)?>>West Virginia</option>
                <option value="WI"<?=setselect('WI', $value)?>>Wisconsin</option>
                <option value="WY"<?=setselect('WY', $value)?>>Wyoming</option>
                <option value="DC"<?=setselect('DC', $value)?>>District of Columbia</option>
            </select>
        </div>
        ...
        <div id="state_field_AU" class="wpuf-fields">
            <select class="form-control" id="state_field_select_AU" name="wpuf_address_state_field_AU" data-required="yes" data-type="select">
                ...
                <option value="NT"<?=setselect('NT', $value)?>>Northern Territory</option>
                <option value="QLD"<?=setselect('QLD', $value)?>>Queensland</option>
                <option value="SA"<?=setselect('SA', $value)?>>South Australia</option>
                <option value="TAS"<?=setselect('TAS', $value)?>>Tasmania</option>
                <option value="VIC"<?=setselect('VIC', $value)?>>Victoria</option>
                <option value="WA"<?=setselect('WA', $value)?>>Western Australia</option>
            </select>
        </div>
    <?php
...
add_action( 'states_fields_hook', 'wpuf_states_fields', 10, 3 );
 ...
 * @param type $post_id
 */
function update_states_fields_hook( $post_id ) {
    if ( isset( $_POST['property_address_suburb'] ) &&  $_POST['property_address_suburb'] != ''){
        $newterm = strtolower(sanitize_text_field($_POST['property_address_suburb']));
        wp_set_object_terms( $post_id, array($newterm), 'location');
    }
    if ( isset( $_POST['wpuf_address_state_field_AU'] ) &&  $_POST['wpuf_address_state_field_AU'] != '') {
        update_post_meta( $post_id, 'property_address_state', $_POST['wpuf_address_state_field_AU'] );
    }else if(isset( $_POST['wpuf_address_state_field_USA'] ) && $_POST['wpuf_address_state_field_USA']  != ''){
        update_post_meta( $post_id, 'property_address_state', $_POST['wpuf_address_state_field_USA'] );
    }
}

add_action( 'wpuf_add_post_after_insert', 'update_states_fields_hook' );
add_action( 'wpuf_edit_post_after_update', 'update_states_fields_hook' );
